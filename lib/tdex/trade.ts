...
import { getFujiCoins, getNextAddress, getNextChangeAddress } from 'lib/marina'
...
const makePset = async (
  ...
  const outputAddress = address.fromConfidential(
    receivingAddress ?? (await getNextAddress()).confidentialAddress,
  )

  updater.addOutputs([
    {
      script: outputAddress.scriptPubKey,
      amount: Number(preview.amount) - Number(preview.feeAmount),
      asset: pair.dest.id,
      blinderIndex: 0,
      blindingPublicKey: outputAddress.blindingKey,
    },
  ])
...
export const proposeTrade = async (
  ...
  const tradeProposeRequest: TDEXv2ProposeTradeRequest = {
    ...
    swapRequest: await createSwapRequest(pair, preview, receivingAddress),
